...
const STRATEGY_CONFIG = {
  slug: "ai-top20-nasdaq100-v1-0-0",
...
const getOrCreateStrategy = async (
  ...
  return {
    ...inserted,
    transaction_cost_bps: toNumber(inserted.transaction_cost_bps, STRATEGY_CONFIG.transactionCostBps),
  } as StrategyRow & { transaction_cost_bps: number };
};
...
const computeQuintileReturns = (samples: ForwardSample[]) => {
  ...
  for (let quintile = 1; quintile <= 5; quintile++) {
    ...
    const avgReturn =
      bucket.reduce((sum, item) => sum + item.forward_return, 0) / bucket.length;
    rows.push({
      quintile,
...
const handleRequest = async (req: Request) => {
  ...
  try {
    ...
    if (!forceRun && runWeekday !== STRATEGY_CONFIG.rebalanceDayOfWeek) {
      log(
        "SKIP",
        `Not rebalance day. run_weekday=${runWeekday}, rebalance_day=${STRATEGY_CONFIG.rebalanceDayOfWeek}`
      );
      return NextResponse.json({
        ...
        runDate,
        runWeekday,
        rebalanceDay: STRATEGY_CONFIG.rebalanceDayOfWeek,
      });
    }

    const supabase = createAdminClient();
    log("CONFIG", `runDate=${runDate}, forceRun=${forceRun}`);
    ...
    if (!nasdaqRows.length) {
      ...
      if (latestSnapshot?.id) {
        ...
        const dbRows = rows
          ...
          .filter((stock): stock is { symbol: string; company_name: string | null } => Boolean(stock))
          .map((stock) => ({
            symbol: stock.symbol,
    ...
    const stockMap = new Map((upsertedStocks || []).map((stock: StockRow) => [stock.symbol, stock]));
    ...
    const results = await chunkWithConcurrency(memberRows, concurrency, async (member) => {
      ...
      if (runError) {
        failed++;
        recordCronError("Supabase analysis upsert failed", runError, `Ticker: ${member.stock.symbol}`);
        return {
          status: "failed",
          stock_id: member.stock_id,
    ...
    const { error: insertHoldingsError } = await supabase.from("strategy_portfolio_holdings").insert(
    ...
    const previousPriceMap =
      previousBatch?.run_date ? await fetchPriceMapForRunDate(supabase, previousBatch.run_date) : new Map<string, number>();
    ...
    const grossReturn = previousHoldings.length
      ...
      : 0;
    const turnover = previousHoldings.length
      ? calculateTurnover(oldWeightMap, newWeightMap)
      : 1;
    ...
    const startingEquity = previousPerf ? toNumber(previousPerf.ending_equity, INITIAL_CAPITAL) : INITIAL_CAPITAL;
    ...
    const equalWeightStart = previousPerf
      ...
      : INITIAL_CAPITAL;
    const sp500Start = previousPerf ? toNumber(previousPerf.sp500_equity, INITIAL_CAPITAL) : INITIAL_CAPITAL;
    ...
    const newMapByStockId = new Map(
      topHoldings.map((holding) => [
        holding.stock_id,
        { stock_id: holding.stock_id, symbol: holding.symbol, target_weight: holding.target_weight },
      ])
    );
    ...
    oldMapByStockId.forEach((holding, stockId) => {
      ...
      actions.push({
        ...
        action_label: exitedIndex
          ? "Exited Index - Sell at next rebalance"
          : "Exited Top-20",
    ...
    if (actions.length) {
      const { error: insertActionsError } = await supabase.from("strategy_rebalance_actions").insert(
        actions.map((action) => ({
          strategy_id: strategy.id,
          run_date: runDate,
