...
import { isoDateInEastern, notifyPortfolioWeeklyRecap } from '@/lib/notifications/portfolio-weekly-recap-cron';
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const weekEndingParam = url.searchParams.get('weekEnding')?.trim();
    const weekEnding =
      weekEndingParam && /^\d{4}-\d{2}-\d{2}$/.test(weekEndingParam)
        ? weekEndingParam
        : isoDateInEastern(new Date());

    const dryUserRaw = url.searchParams.get('dryUser')?.trim() ?? '';
    ...
    const summary = await notifyPortfolioWeeklyRecap(admin, { weekEnding, dryUserId });
