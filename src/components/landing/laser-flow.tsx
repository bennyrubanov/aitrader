...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const onMoveListener: EventListener = (ev) => {
      if (!('clientX' in ev) || !('clientY' in ev)) return;
      onMove(ev as PointerEvent);
    };
    const onLeave = () => mouseTarget.set(0, 0);
    const onLeaveListener: EventListener = () => onLeave();
    canvas.addEventListener('pointermove', onMoveListener, { passive: true });
    canvas.addEventListener('pointerdown', onMoveListener, { passive: true });
    canvas.addEventListener('pointerenter', onMoveListener, { passive: true });
    canvas.addEventListener('pointerleave', onLeaveListener, { passive: true });
    ...
    return () => {
      ...
      canvas.removeEventListener('pointermove', onMoveListener);
      canvas.removeEventListener('pointerdown', onMoveListener);
      canvas.removeEventListener('pointerenter', onMoveListener);
      canvas.removeEventListener('pointerleave', onLeaveListener);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- GL init only on `dpr`; uniforms sync in next effect
  }, [dpr]);
  ...
  return <div ref={mountRef} className={cn('relative h-full w-full', className)} style={style} />;
};
