...
export const STRATEGY_MODELS_RANKED_CACHE_TAG = PUBLIC_CACHE_TAGS.strategyModelsRanked;
...
async function loadStrategyModelsRanked(): Promise<RankedStrategyModel[]> {
  ...
  const withScores: RankedStrategyModel[] = rows.map((r, i) => {
    const modelScore =
      0.5 * breadthNorm[i]! + 0.3 * medianNorm[i]! + 0.2 * bestNorm[i]!;
...
const getStrategyModelsRankedCached = unstable_cache(
  loadStrategyModelsRanked,
  ['strategy-models-ranked'],
  {
    revalidate: PUBLIC_DATA_CACHE_TTL_SECONDS,
    tags: [RANKED_CONFIGS_CACHE_TAG, STRATEGY_MODELS_RANKED_CACHE_TAG],
  }
);

/** Cross-strategy ranked stats for `/strategy-models` (same payload as the public API). */
export async function getStrategyModelsRanked(): Promise<RankedStrategyModel[]> {
  return getStrategyModelsRankedCached();
}
