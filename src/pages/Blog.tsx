          ...
          <div className="container mx-auto px-4">
            <div className="max-w-5xl mx-auto">
              ...
              <p className="text-xl text-gray-600 text-center mb-16 max-w-3xl mx-auto">
                Insights on AI-driven trading, market analysis, and research-backed investment strategies.
              </p>
              
              <div className="grid md:grid-cols-2 gap-8">
                {blogPosts.map(post => (
                  <article key={post.id} className="bg-white shadow-soft rounded-xl overflow-hidden hover:shadow-elevated transition-shadow duration-300">
                    <Link to={`/blog/${post.id}`} className="block">
                      <img src={post.image} alt={post.title} className="w-full h-52 object-cover" />
                      <div className="p-6">
                        <span className="text-sm text-gray-500">{post.date}</span>
                        <h2 className="text-xl font-bold mt-2 mb-3 hover:text-trader-blue">{post.title}</h2>
