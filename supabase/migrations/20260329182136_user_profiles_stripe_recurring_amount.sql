-- Snapshot of primary subscription item unit amount + currency for billing UI (Stripe smallest unit, e.g. cents).

alter table public.user_profiles
  add column if not exists stripe_recurring_unit_amount integer,
  add column if not exists stripe_recurring_currency text;
