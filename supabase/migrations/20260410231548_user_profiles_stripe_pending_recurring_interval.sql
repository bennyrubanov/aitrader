-- Target billing cadence when a same-tier interval change is scheduled (Stripe subscription schedule).

alter table public.user_profiles
  add column if not exists stripe_pending_recurring_interval text;

alter table public.user_profiles
  drop constraint if exists user_profiles_stripe_pending_recurring_interval_valid;

alter table public.user_profiles
  add constraint user_profiles_stripe_pending_recurring_interval_valid check (
