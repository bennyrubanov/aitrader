...
create table if not exists public.user_profiles (
  ...
  sign_in_client_fingerprints jsonb not null default '[]'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  ...
  constraint user_profiles_sign_in_fingerprints_is_array check (
...
-- Signed-in client: last context, per-device counts, fingerprint set, optional in-app security.new_sign_in (POST /api/auth/record-sign-in-context).
...
-- =========================
-- 14c) Daily config/strategy series snapshots (current + history)
-- =========================
...
create table if not exists public.portfolio_config_daily_series (
  ...
  data_status text not null default 'ready'
    check (data_status in ('ready', 'in_progress', 'failed', 'empty')),
...
create table if not exists public.user_portfolio_profiles (
  ...
  portfolio_notify_email_bits smallint not null default 0,
  portfolio_notify_inapp_bits smallint not null default 0,
